...
func TestRecordPipelineRunDurationCount(t *testing.T) {
	...
	testData := []struct {
		...
		taskRun: tb.PipelineRun("pipelinerun-1",
...
func TestRecordRunningPipelineRunsCount(t *testing.T) {
	...
	addPipelineRun(informer, "pipelinerun-1", "pipeline-1", corev1.ConditionTrue, t)
	addPipelineRun(informer, "pipelinerun-2", "pipeline-2", corev1.ConditionFalse, t)
	addPipelineRun(informer, "pipelinerun-3", "pipeline-3", corev1.ConditionUnknown, t)
...
func addPipelineRun(informer alpha1.PipelineRunInformer, run, pipeline string, status corev1.ConditionStatus, t *testing.T) {
	t.Helper()

	err := informer.Informer().GetIndexer().Add(tb.PipelineRun(run,
